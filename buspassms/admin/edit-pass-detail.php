...
if (strlen($_SESSION['bpmsaid']==0)) {
  ...
  if(isset($_POST['submit'])) {
    ...
    $cnum=$_POST['cnumber'];
    $email=$_POST['email'];
    $studentidcard=$_POST['identitytype'];
    $aadhar=$_POST['aadhar'];
    $source=$_POST['source'];
    ...
    $lastFourDigits = substr($aadhar, -4);
    $passnum = $initial . $firstTwoLetters . $lastFourDigits;

    $sql="update tblpass set PassNumber=:passnum, FullName=:fname, ContactNumber=:cnum, Email=:email, IdentityType=:studentidcard, IdentityCardno=:aadhar, Source=:source, Destination=:des, FromDate=:fdate, ToDate=:tdate, Cost=:cost where ID=:eid";
    ...
    $query->bindParam(':studentidcard',$studentidcard,PDO::PARAM_STR);
...
{ ?>
 ...
 Pass ID: <?php echo ($row->PassNumber);?></p>
    ...
    <div class="form-group"> <label for="exampleInputEmail1">Photo</label> <img src="../uploadimg/<?php echo $row->ProfileImage;?>" width="50" height="50" value="<?php echo $row->ProfileImage;?>">
<a href="changeimage.php?editid=<?php echo $row->ID;?>"> &nbsp; Edit Image</a> </div>
    ...
    <div class="form-group"> <label for="exampleInputEmail1">Aadhar Card</label> <input type="aadhar" name="aadhar" value="<?php echo $row->aadhar_card;?>" class="form-control" required='true'> </div>
...
<option value="Any Other Govt Issued Doc">Any Other Govt Issued Doc</option>
    ...
    <!--<div class="form-group"> <label for="exampleInputEmail1">Identity Card No.</label> <input type="text" name="aadhar" value="<?php echo $row->IdentityCardno;?>" class="form-control" required='true'> </div> -->
